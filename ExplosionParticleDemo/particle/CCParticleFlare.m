...
-(void) updateParticleSpeed:(tCCParticle *)p elapsed:(float)e dt:(float)dt {
    p->mode.A.speed = CC_LINEAR(-10, 150, elapsed);
}

-(void) updateParticleColor:(tCCParticle *)p elapsed:(float)e dt:(float)dt {
    if (elapsed < 0.5f)
        p->color.a = CC_LINEAR(2, 0, elapsed);
    else
        p->color.a = CC_LINEAR(-2, 1, elapsed);
}
@end
...
-(void) updateParticleSize:(tCCParticle *)p elapsed:(float)e dt:(float)dt {
    p->size = p->orgSize * CC_LINEAR(-1, 1.2f, elapsed);
}

-(void) updateParticleColor:(tCCParticle *)p elapsed:(float)elapse dt:(float)dt {
    if (elapsed <= 0.1) {
        p->color.a = CC_LINEAR(0.1, 0, elapsed);
    }
    else {
        p->color.a = 1.0f;
    }
...
-(void) updateParticleSpeed:(tCCParticle *)p elapsed:(float)e dt:(float)dt {
    if (elapsed <= 0.03) {
        p->mode.A.speed = p->mode.A.orgSpeed * CC_LINEAR(-11, 0.45f, elapsed);
    }
    else {
        p->mode.A.speed = p->mode.A.orgSpeed * CC_LINEAR(-0.12, 0.12f, elapsed);
    }
}
@end
...
-(void) updateParticleSpeed:(tCCParticle *)p elapsed:(float)e dt:(float)dt {
    p->mode.A.speed = p->mode.A.orgSpeed * CC_LINEAR(-0.4f, 1.0f, elapsed);
}
@end
